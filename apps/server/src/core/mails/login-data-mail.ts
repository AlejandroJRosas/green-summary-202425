export function constructLoginDataMail(email: string, password: string) {
  return `
  <main style="height: 100%; padding: 0 40px; display: flex; flex-direction: column; color: #323232; font-family: 'Arial';">
  <div style="padding: 15px 0px; color: #323232; font-family: 'Arial';">
    <h1 style="color: #323232; font-weight: 500; font-family: 'Arial';">
      Green Summary
    </h1>
  </div>

  <div style="background-color: #00ad7c; height: 3.5px; display: block; color: #323232; font-family: 'Arial';"></div>

  <h1 style="color: #323232; font-size: 25px; text-align: center; margin-top: 55px; font-family: 'Arial';">
    Tus datos para iniciar sesión
  </h1>

  <p style="display: block; font-size: 17px; margin-top: 30px; margin-right: 40px; margin-left: 40px; color: #323232; font-family: 'Arial';">
    A continuación se presentan tus datos que puedes utilizar para iniciar sesión en la plataforma <i>Green Summary</i>.
  </p>

  <section style="width: fit-content; margin: auto; margin-top: 60px; display: flex; flex-direction: column; gap: 30px; border: 2px solid whitesmoke; padding: 4% 5%; border-radius: 10px; color: #323232; font-family: 'Arial';">
    <div style="display: flex; flex-direction: column; gap: 4px; color: #323232; font-family: 'Arial';">
      <b style="color: #323232; font-family: 'Arial';">Correo electrónico</b>
      <i style="color: #323232; font-family: 'Arial';">${email}</i>
    </div>

    <div style="display: flex; flex-direction: column; gap: 4px; color: #323232; font-family: 'Arial';">
      <b style="color: #323232; font-family: 'Arial';">Contraseña</b>
      <i style="color: #323232; font-family: 'Arial';">${password}</i>
    </div>
  </section>
</main>
  `
}
